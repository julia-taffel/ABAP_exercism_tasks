...
CLASS zcl_itab_nesting IMPLEMENTATION.
  METHOD perform_nesting.
    DATA: ls_nes LIKE LINE OF nested_data,
          ls_alb LIKE LINE OF ls_nes-albums.
    FIELD-SYMBOLS: <art> LIKE LINE OF artists,
                   <alb> LIKE LINE OF albums,
                   <song> LIKE LINE OF songs,
                   <nes> LIKE LINE OF nested_data,
                   <nes_alb> LIKE LINE OF ls_nes-albums,
                   <nes_song> LIKE LINE OF ls_alb-songs.

    LOOP AT artists ASSIGNING <art>.
      APPEND INITIAL LINE TO nested_data ASSIGNING <nes>.
      <nes>-artist_id   = <art>-artist_id.
      <nes>-artist_name = <art>-artist_name.

      LOOP AT albums ASSIGNING <alb> WHERE artist_id = <art>-artist_id.
        APPEND INITIAL LINE TO <nes>-albums ASSIGNING <nes_alb>.
        <nes_alb>-album_id    = <alb>-album_id.
        <nes_alb>-album_name  = <alb>-album_name.

        LOOP AT songs ASSIGNING <song> WHERE artist_id = <art>-artist_id
                                         AND album_id  = <alb>-album_id.
          APPEND INITIAL LINE TO <nes_alb>-songs ASSIGNING <nes_song>.
          <nes_song>-song_id     = <song>-song_id.
          <nes_song>-song_name   = <song>-song_name.
        ENDLOOP.
      ENDLOOP.
    ENDLOOP.        
  ENDMETHOD.
